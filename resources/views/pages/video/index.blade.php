...
<html lang="en" dir="ltr">
  ...
  <body class="sidebar-mini">
    ...
    <script>
        ...
        // Get Image
        firebase.database().ref('Video/').orderByChild('desaId').equalTo("{{ Auth::id() }}").on('value', function (snapshot) {
            var value = snapshot.val();
            var htmls = [];
            if (value != null) {
                ...
                $.each(value, function (index, value) {
                    if (value) {
                        htmls.push('<tr>\
                    <td>' + value.video + '</td>\
                    <td><video width="320" height="240" controls> <source src="' + value.url + '" type="video/mp4"> </video> </td>\
                    <td><button data-toggle="modal" data-target="#update-modal" class="btn btn-info updateData" data-id="' + index + '">Update</button>\
                    <button data-toggle="modal" data-target="#remove-modal" class="btn btn-danger removeData" data-id="' + index + '">Delete</button></td>\
                  </tr>');
                  old_image = value.video;
                    }
                });
            }else{
        ...
        // Listen for state changes, errors, and completion of the upload.
          uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, // or 'state_changed'
              ...
              uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                ...
                firebase.database().ref('Video/' + userID).set({
                    desaId: "{{Auth::id()}}",
                    video: file.name,
                    url: downloadURL,
                });
        ...
        $('body').on('click', '.updateData', function () {
            ...
            firebase.database().ref('Video/' + updateID).on('value', function (snapshot) {
                var values = snapshot.val();
                var updateData =
                '<div class="form-group">\
                    <label for="image_update" class="col-md-12 col-form-label">Image</label>\
                    <div class="col-md-12">\
                        <input id="image_update" type="text" class="form-control" name="updateimage" value="' + values.video + '" readonly disabled required autofocus>\
                    </div>\
                </div>\
                <div class="form-group">\
        ...
        $('#editform').on('submit', function (e) {
          ...
          uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, // or 'state_changed'
              ...
              uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                ...
                var postData = {
                    video: file_update.name,
                    url: downloadURL,
                    desaId: "{{ Auth::id() }}",
                };

                var updates = {};
                updates['/Video/' + updateID] = postData;
        ...
        $('.deleteRecord').on('click', function () {
            ...
            firebase.database().ref('Video/' + id + '/').on('value', function (snapshot) {
              var value = snapshot.val();
                if (value) {
                    image = value.video;
                  }
            });
            firebase.database().ref('Video/' + id).remove();
