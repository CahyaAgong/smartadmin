...
<html lang="en" dir="ltr">
  ...
  <body class="sidebar-mini">
    ...
    <script>
        ...
        var path = " {{ Auth::user()->name }}/Foto/";

        var old_image = "";

        //set index
        firebase.database().ref('Foto/').on('value', function (snapshot) {
        ...
        // Get Image
        firebase.database().ref('Foto/').orderByChild('desaId').equalTo("{{ Auth::id() }}").on('value', function (snapshot) {
            var value = snapshot.val();
            var htmls = [];
            if (value != null) {
        ...
        // Listen for state changes, errors, and completion of the upload.
          uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, // or 'state_changed'
              ...
              uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                ...
                firebase.database().ref('Foto/' + userID).set({
                    desaId: "{{Auth::id()}}",
                    image: file.name,
                    url: downloadURL,
                });
        ...
        $('body').on('click', '.updateData', function () {
            ...
            firebase.database().ref('Foto/' + updateID).on('value', function (snapshot) {
        ...
        $('.updateFoto').on('click', function () {
          ...
          uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, // or 'state_changed'
              ...
              uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                ...
                var updates = {};
                updates['/Foto/' + updateID] = postData;
        ...
        $('.deleteRecord').on('click', function () {
            ...
            firebase.database().ref('Foto/' + id + '/').on('value', function (snapshot) {
              var value = snapshot.val();
                if (value) {
                    image = value.image;
                  }
            });
            firebase.database().ref('Foto/' + id).remove();
